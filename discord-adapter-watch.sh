...
set -uo pipefail
A=/var/lib/hermes/.hermes/hermes-agent/plugins/platforms/discord/adapter.py
STATE=/var/lib/hermes/.hermes/state/discord-adapter-watch.state
...
ver="$(cd /var/lib/hermes/.hermes/hermes-agent && git describe --tags --always 2>/dev/null || echo unknown)"
...
if [ "${now}" != "${prev}" ]; then
    ...
    echo "  was: ${prev}"
    echo "  now: ${now}"
    echo "  Re-check /var/lib/hermes/.hermes/plugins/discord-ambient/adapter.py against the bundled adapter — see Wiki/Fleet.md 'Discord ambient presence'."
fi
exit 0
