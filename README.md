...
| **Gateway lifecycle messages** | zero inference | On a graceful gateway drain, post one configured in-character departure in the private/home channel before Discord disconnects. On the next initial connection, post one return there and optionally roll for a second public return. Transport reconnects emit neither message. A hard crash or power loss cannot reliably announce itself. |
...
| **Discord mention resolution** | **zero** | `resolve_plain_mentions` converts a model's plain `@display-name` to Discord's real `<@numeric-id>` wire form only when Discord itself supplied an unambiguous identity in that channel. Unknown or colliding names stay plain text rather than pinging the wrong person. Catch-up transcripts contribute their observed participants too. |
    ...
    extra:
      ambient_presence:
        ...
        gateway_lifecycle:
          ...
          departure_messages: ["The shrine is quieting; I'm going to sleep."]
          return_messages: ["The shrine flame stirs; I'm awake again."]
          public_return:
            channel: "<public-channel-id>"
            probability: 0.5       # independent roll on each initial gateway connect
            messages: ["A familiar tail curls around the doorway. I'm back."]
        embed_media:
